...
### 2. UCache - 智能文件缓存
- **ARC 算法**：Adaptive Replacement Cache，自适应替换缓存
- **双重 LRU**：T1 (最近访问) + T2 (频繁访问)
- **幽灵列表**：B1/B2 记录历史，动态调整分割点
- **自适应分割**：根据访问模式自动调整 T1/T2 比例
- **统计信息**：实时监控命中率和缓存分布
...
- **自适应能力**：混合负载下比 LRU 提升 10-30%
- **内存占用**：256 条目默认配置 + 幽灵列表
- **延迟降低**：缓存命中时减少 70-90% I/O 延迟

**ARC 优势**:
- 同时考虑**最近性**(Recency)和**频繁性**(Frequency)
- 自动适应工作负载变化(扫描、循环访问、随机访问)
- 无需手动调参
