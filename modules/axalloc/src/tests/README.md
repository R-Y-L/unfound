...
# UnfoundOS Allocator Testing Framework

This document describes the comprehensive testing framework for memory allocators in UnfoundOS.

## Overview

The testing framework provides modular, extensible tests for evaluating memory allocator performance, fragmentation, stability, and time-dimension behavior. It supports:

- **Basic Performance Tests**: Success rate, allocation/deallocation timing, throughput
- **Fragmentation Tests**: External/internal fragmentation, metadata overhead
- **Stability Tests**: Failure rates, memory leak detection, multi-thread contention
- **Time-Dimension Tests**: Long-running simulation (1h, 6h, 24h)
- **Runtime Switch Tests**: Allocator switching latency and compatibility
- **Kernel-Compatible Tests**: no_std compatible for embedded/kernel environments
...
```bash
# Run complete test suite with buddy allocator
cargo run --bin allocator_test --features "buddy std" all
...
```
Usage: allocator_test [OPTIONS] [WORKLOAD]

Workloads:
  all, complete        Run complete test suite (default)
  basic                Run basic performance tests
  fragmentation        Run fragmentation tests
  stability            Run stability tests
  time-dimension       Run time-dimension tests
  legacy               Run legacy workload tests
...
let allocator = BuddyAllocator::new();
allocator.init(0x1000, 256 * 1024 * 1024).unwrap(); // 256MB
...
let allocator = BuddyAllocator::new();
allocator.init(0x1000, 1024 * 1024 * 1024).unwrap(); // 1GB
...
- `small`: Quick validation (64MB pool, 1000 allocations)
- `medium`: Standard testing (256MB pool, 10000 allocations)
- `large`: Stress testing (1GB pool, 100000 allocations)
...
config.basic_perf.num_allocations = 50000;
config.basic_perf.measure_individual_times = true;
config.verbose = true;
```
...
> **Note**: Time units are auto-selected based on magnitude:
> - < 1μs → displayed as **ns** (nanoseconds)
> - 1μs ~ 1ms → displayed as **μs** (microseconds)
> - ≥ 1ms → displayed as **ms** (milliseconds)

### Allocator Performance Comparison

| Allocator | Success Rate | Avg Alloc | Avg Dealloc | Throughput | P50 | P99 |
|-----------|--------------|-----------|-------------|------------|-----|-----|
| **Buddy** | 64.6% | 569ns | 1.28μs | 909K/s | 509ns | 1.4μs |
| **Bitmap** | 66.3% | 16.4μs | 339ns | 55K/s | 5.7μs | 147μs |
| **Hybrid** | 73.6% | 157μs | 1.16μs | 13K/s | 86μs | 640μs |

> **Note**: Data based on 1GB memory pool, 10000 allocations (mixed alloc/dealloc pattern with 30% dealloc ratio)
...
The testing framework is designed to work with any allocator implementing the `PageAllocator` trait:
...
pub trait PageAllocator: Send + Sync {
    ...
    fn alloc_pages_at(&self, start: usize, num_pages: usize, align_pow2: usize) -> Result<usize, AllocError>;
    fn dealloc_pages(&self, pos: usize, num_pages: usize);
    fn get_stats(&self) -> (f64, usize); // (fragmentation_rate, total_free_bytes)
}
```

To test a new allocator:

1. Implement `PageAllocator` trait
2. Add feature flag in Cargo.toml if needed
3. Run tests using the framework
...
├── metrics.rs          # Data structures for test results
├── config.rs           # Test configuration parameters
├── suites.rs           # Main test suite implementations
├── time_dimension.rs   # Long-running time-dimension tests
├── kernel_tests.rs     # no_std compatible kernel tests
├── allocator_tester.rs # Legacy tester (backward compatibility)
└── workloads.rs        # Legacy workloads (backward compatibility)
```

## Features

- `std`: Enable std-based testing with full features
- `buddy`: Enable buddy allocator
- `bitmap`: Enable bitmap allocator
- `hybrid`: Enable hybrid allocator
- `runtime-switch`: Enable runtime allocator switching tests
- `page-alloc-4g`: Support up to 4GB memory pool (default, 1M pages)
- `page-alloc-64g`: Support up to 64GB memory pool (16M pages)
- `page-alloc-256m`: Support up to 256MB memory pool (64K pages, for embedded)
